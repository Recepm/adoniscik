...
const fronz = require("./ayarlar/bot.json"); // mal buraya neden fronz yaptın amk 
...
.setThumbnail()
.addField(`Adonis | Teşekkürler`, `**Selamlar, Ben Fronz (Adonis'in Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`)
.addField(`Adonis | Prefix`, `**Adonis Botun Prefixi(ön eki) = \`${fronz.prefix}\`\n\n Değiştirebilmek için \`${fronz.prefix}prefix\` Yazabilirsiniz.**`)
.addField(`Adonis | Nasıl Kullanılır?`, `**Adonis botun tüm özelliklerinden yararlanabilmek için sadece \`${fronz.prefix}yardım\` yazmanız yeterlidir.**`)
.addField(`Adonis | Linkler`, `**Sohbet Kanalına a!davet Yazmanız Yeterlidir**`)
...
//const player = new Player(client, fronz.youtube_api); bakılacak
...
// `✨ Yeni Özellikler İçin | a!yenilikler`,
// `⚡️ Botu eklemek için | a!davet`,
// `🌟 Prefix ayarlamak için | a!prefix`
// ]
...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "a!yardım | a!davet" } });
});  
...
client.on('message', async msg => {
  let prefix = await db.fetch(`prefix.${msg.guild.id}`) || fronz.prefix 
  if(msg.content == `<@!765535600291610624>`) return msg.channel.send(`> **Adonis | Prefix**\n\n> <a:hypesquad:765831981568622602> **Sanırım beni etiketlediniz.**\n > <a:hypesquad:765831981568622602> Buyurun prefix(ön ek)im \`${prefix}\``);
});
...
.setDescription(`<a:hypesquad:765831981568622602> <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("Adonis Bot | Log Sistemi")
...
if (!modlogkanal) return;
  ...
  .setColor("#ffd100")
    ...
    .setDescription(`<a:hypesquad:765831981568622602> **Üye Sunucudan Yasaklandı!** \n<@!${message.user.id}>, ${message.user.tag}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setDescription(`<a:hypesquad:765831981568622602> ${channel.name} **Adlı Metin Kanalı Oluşturuldu!**`)
                .setFooter(`Adonis Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
...
.setTitle("SES KANALI OLUŞTURULDU")
                .setDescription(`<a:hypesquad:765831981568622602> ${channel.name} **Adlı Ses Kanalı Oluşturuldu!**`)
                .setFooter(`Adonis Bot | Log Sistemi Kanal ID: ${channel.id}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                let embed = new Discord.MessageEmbed()
                    .setColor('#ffd100')
                .setDescription(`<a:hypesquad:765831981568622602> ${channel.name} **Adlın Metin Kanalı  Silindi**`)
                .setFooter(`Adonis Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
            if (channel.type === "voice") {
...
.setTitle("SES KANALI SİLİNDİ")
                .setDescription(`<a:hypesquad:765831981568622602> ${channel.name} **Adlı Ses Kanalı Silindi**`)
            .setFooter(`Adonis Bot | Log Sistemi  Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            }
    })
...
client.on("message", async message => {
  ...
  const lus = await db.fetch(`argoengel_${message.guild.id}`)
  if (lus) {
    ...
    if (küfür.some(word => message.content.toLowerCase().startsWith(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          message.delete();
          
          return message.channel.send(`<a:hypesquad:765831981568622602> **Hey ${message.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`)
...
client.on("messageUpdate", async (newMessage, oldMessage) => {
  ...
  const lus = await db.fetch(`argoengel_${newMessage.guild.id}`)
  if (lus) {
 ...
 if (küfür.some(word => oldMessage.content.toLowerCase().includes(word)))
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          
          return newMessage.channel.send(`<a:hypesquad:765831981568622602> **${message.author}, mesajını editlesende küfürünü engellerim! Bu sunucuda küfür edemezsin!**`)
		  .then(message => message.delete(2000));
    ...
    if (reklamengel.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`<a:hypesquad:765831981568622602> **Hey ${message.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`)
		  .then(message => message.delete(2000));
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`<a:hypesquad:765831981568622602> **Hey ${newMessage.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`)
		  .then(message => message.delete(2000));
    ...
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'sa' || msg.content.toLowerCase() == 's.a' || msg.content.toLowerCase() == 'selamun aleyküm' || msg.content.toLowerCase() == 'sea' || msg.content.toLowerCase() == 's.a.' || msg.content.toLowerCase() == 'selam' || msg.content.toLowerCase() == 'slm') {
          try {
                  ...
                  return msg.reply('**<a:hypesquad:765831981568622602> Aleyküm Selam, Hoşgeldin.** ')
          } catch(err) {
            console.log(err);
          }
      }
...
client.on("guildMemberAdd", async member => {
  ...
  ///....
  if (!mesaj) {
    return client.channels.cache.get(kanal).send("<a:hypesquad:765831981568622602> `"+ member.user.username + "`**Adlı Kullanıcı Aramıza Katıldı!** `" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
  }

  if (member.guild.memberCount == sayaç) {
    return client.channels.get(kanal).send(`<a:hypesquad:765831981568622602> **Sayaç Sıfırlandı!** \`${member.guild.memberCount}\` **Kişiyiz!**`)
...
client.on("guildMemberRemove", async member => {
  ...
  if (!mesaj) {
    return client.channels.cache.get(kanal).send("<a:hypesquad:765831981568622602> `" + member.user.username + "` **Adlı Kullanıcı Aramızdan Ayrıldı.**`" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
      }
...
client.on("guildMemberAdd", async member => {
  ...
  if (!mesaj) {
    client.channels.cache.get(kanal).send("<a:hypesquad:765831981568622602> **Selam!** `" + member.user.username + "`**!kayıtol yazarak kayıt olabilirsin!**");
...
let pieceKick = new Discord.MessageEmbed()
...
.addField("Sunucudaki Kişi Sayısı:", guild.memberCount)
   ...
   client.channels.cache.get('766663400654503956').send(pieceKick);
...
let pieceEnter = new Discord.MessageEmbed()
...
.addField("Sunucudaki Kişi Sayısı:", guild.memberCount)
   ...
   client.channels.cache.get('766663400654503956').send(pieceEnter);
...
client.on("message", async msg => {
  ...
  let i = await db.fetch(`üyelikk_${msg.author.id}`);
  if (i === 'üyelik') {
    if (dakdest !== null && timeout - (Date.now() - dakdest) > 0) {
      ...
      if (msg.content.length > 1) {
        ...
        var embed = new Discord.RichEmbed()
          .setDescription(
            `<a:se:757334263392764074> Herkes dursun! Burada gold ayrıcalığına sahip bir üye belirdi \n<@${msg.author.id}> <a:se:757334263392764074>`
          )
          .setColor("GOLD");
