...
if (!args[0])  {
  ...
  if (args [0] == 'aç') {
    db.set(`argoengel_${message.guild.id}`, '<a:hypesquad:765831981568622602> **Küfür Engel Aktif!**')
    let lus = await db.fetch(`argoengel_${message.guild.id}`)
    
    const reklamengelcim = new Discord.MessageEmbed()
    .setTitle('Başarılı')
      .setColor("#0affff")
    .setDescription('<a:hypesquad:765831981568622602> **Küfür Engel Başarıyla Açıldı**')
  ...
  if (args [0] == 'kapat') {
    ...
    db.delete(`argoengel_${message.guild.id}`)

   const küfürengelcim22 = new Discord.MessageEmbed()
    .setTitle('Başarılı')
    .setDescription('<a:hypesquad:765831981568622602> **Küfür Engel Başarıyla Kapatıldı**')
