...
.setDescription(`<a:hypesquad:765831981568622602>  **| Müzik oynatabilmek için bir ses kanalında olmanız gerekmektedir!**`)
...
.setDescription(`<a:hypesquad:765831981568622602>  **| Müzik oynatabilmek için aranacak kelime veyahut kelimeler giriniz!**`)
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2020`, client.user.avatarURL());    
...
const sarki = await client.player.addToQueue(message.guild.id, args.join(" "));
  ...
  .setColor(AloneDogru)
  .setTitle("Adonis | Kuyruğa Ekle")
  .setDescription(`<a:hypesquad:765831981568622602> **|** \`${sarki.name}\` **adlı müzik,**\n${message.author} **tarafından kuyruğa eklendi!**`)
.setThumbnail(`https://drummofficial.com/wp-content/uploads/2017/08/equalizer3.gif`)
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2020`, client.user.avatarURL());   
...
const sarki = await client.player.play(message.member.voice.channel, args.join(" "));
  ...
  .setColor(AloneDogru)
  .setTitle("Adonis | Oynatılan Şarkı")
  .setDescription(`<a:hypesquad:765831981568622602> **|** \`${sarki.name}\` **adlı müzik şu anda oynatılıyor.**`)
...
sarki.queue.on('end', () => {
  ...
  .setColor(AloneDogru)
  .setTitle("Adonis | Kuyruk Bitti!")
  .setDescription(`<a:hypesquad:765831981568622602> **| Kuyruktaki tüm müzikler oynatıldı. Adonis kanaldan ayrılıyor.\n\Adonis'i Tercih ettiğiniz için teşekkür ederiz.**`)
  .setThumbnail(client.user.avatarURL())
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2020`, client.user.avatarURL());  
