...
const db = new qdb.table("bot");
const conf = require('../ayarlar/bot.json');

exports.run = async(client, message, args, ayar, emoji) => {
  if(!conf.sahip.some(id => message.author.id === id)) return;
  if (!args[0] || args[0].includes('token')) return message.channel.send("Kod belirtilmedi `" + this.config.name + "`__`<kod>`__")
